...
export const usePendingPayments = (): UseQueryResult<
  AdminPaymentListResponse,
  Error
> => {
  return useQuery({
...
export const useAdminPayment = (
  id: string,
): UseQueryResult<AdminPayment, Error> => {
  return useQuery({
    queryKey: ["adminPayment", id],
    enabled: !!id,
    queryFn: async () => {
      const response = await ERDEAxios.get<AdminPayment>(
...
export const useRejectAdminPayment = () => {
  ...
  return useMutation({
    mutationFn: async ({ id, reason }: { id: string; reason: string }) => {
      const response = await ERDEAxios.post<AdminPayment>(
        `/transaction/admin/${id}/reject`,
        {
          reason,
        },
      );
