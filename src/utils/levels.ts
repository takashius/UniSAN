...
export function getLevelCaps(levels: SanLevel[] | undefined, level: number): number[] {
  const config = (levels ?? []).find((item) => item.level === level);
  return config?.amounts?.length ? [...config.amounts].sort((a, b) => a - b) : [];
}

/** Tope más alto del nivel: un san califica si no lo pasa. */
export function getMaxCap(levels: SanLevel[] | undefined, level: number): number | null {
...
export function getLevelType(level: number, maxLevel: number | null): LevelType {
