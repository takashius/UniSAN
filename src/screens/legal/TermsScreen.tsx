...
const TermsScreen: React.FC<TermsScreenProps> = ({ mode = 'read' }) => {
  ...
  const { data, isPending, isError, refetch, isFetching } = usePublicTerms(i18n.language);
  ...
  return (
    <View style={[styles.container, mode === 'accept' ? { paddingTop: insets.top + 12 } : null]}>
      <FullScreenLoader visible={acceptTerms.isPending} />
      {mode === 'accept' ? <Text style={styles.gateTitle}>{t('Terms.title')}</Text> : null}
      {mode === 'accept' ? <Text style={styles.gateSubtitle}>{t('Terms.mustAccept')}</Text> : null}
      ...
      {isPending ? (
        ...
        <View style={styles.centered}>
          ...
          <Button textColor="#ff7f50" onPress={() => void refetch()} loading={isFetching}>
            {t('common.retry')}
          </Button>
        </View>
      ) : (
